...
class MANO(nn.Module):
    ...
    def __init__(self,
        ...
        joint_ids = to_tensor(list(TIP_IDS['mano'].values()), dtype=torch.long).to(self.posedirs.device)
        posedirs = self.posedirs.view(self.posedirs.shape[0], -1, 3)
        posedirs = torch.index_select(posedirs, 1, joint_ids)
        posedirs = posedirs.view(posedirs.shape[0], -1)
        self.selected_posedirs = posedirs
        #self.selected_posedirs = posedirs.cuda() # TODO
        W = self.lbs_weights.unsqueeze(dim=0).expand([batch_size, -1, -1])
        W = torch.index_select(W, 1, joint_ids)
        self.selected_W = W
        #self.selected_W = W.cuda() # TODO
    ...
    def get_shape_var(self, betas):
        joint_ids = to_tensor(list(TIP_IDS['mano'].values()), dtype=torch.long).to(betas.device)
        v_shaped = self.v_template + blend_shapes(betas, self.shapedirs)
        J = vertices2joints(self.J_regressor, v_shaped)
        v_shaped = torch.index_select(v_shaped, 1, joint_ids)
        return J, v_shaped

    def forward(self, betas=None, global_orient=None, hand_pose=None, transl=None,
                return_verts=True, return_tips = False, return_full_pose=False, pose2rot=True, J=None, v_shaped=None,
                **kwargs):
        '''
        '''
        ...
        if return_verts:
            vertices, joints = lbs(betas, full_pose, self.v_template,
                                   self.shapedirs, self.selected_posedirs,
                                   self.J_regressor, self.parents,
                                   self.selected_W, pose2rot=pose2rot,
                                   J=J, v_shaped=v_shaped,
                                   dtype=self.dtype)
